 ...
 */
class MO_StreamLoad(bufSize: Int, tensorType: String = "none", num_out: Int ,debug: Boolean = false)(
  ...
  val io = IO(new Bundle {
    ...
    val out = Decoupled(Vec(num_out, UInt(p(XLEN).W)))
  })
  ...
  val queue = Module(new MIMOQueue(UInt(p(XLEN).W), entries = bufSize, tp.tensorWidth, NumOuts = num_out))
  val RegQueue = Module(new MIMOQueue(UInt(p(XLEN).W), entries = num_out, num_out, NumOuts = num_out, pipe=true))

  // connect queue to RegQueue
  queue.io.deq <> RegQueue.io.enq

  RegQueue.io.clear := false.B
  ...
  // original direct queue
//  for(i <- 0 until num_out){
//    io.out.bits(i) := queue.io.deq.bits(i).asUInt()
//  }
//  io.out.valid := queue.io.deq.valid
//  queue.io.deq.ready := io.out.ready

  for(i <- 0 until num_out){
    io.out.bits(i) := RegQueue.io.deq.bits(i).asUInt()
  }
  io.out.valid := RegQueue.io.deq.valid
  RegQueue.io.deq.ready := io.out.ready
