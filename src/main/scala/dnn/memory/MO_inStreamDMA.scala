...
// NOTES: This module is similar to inStreamDMA. but this supports multiple output
 ...
 */
class MO_inStreamDMAIO(memTensorType: String = "none", num_outs: Int)(implicit val p: Parameters)
  extends Module {
  ...
  val io = IO(new Bundle {
    ...
    val out = Decoupled(Vec(num_outs, UInt(p(XLEN).W)))
  })
}

class MO_inStreamDMA(bufSize: Int, memTensorType: String = "none", num_out: Int)(implicit p: Parameters)
  extends MO_inStreamDMAIO(memTensorType, num_out)(p) {

  val strLoad = Module(new MO_StreamLoad(bufSize, memTensorType, num_out))
