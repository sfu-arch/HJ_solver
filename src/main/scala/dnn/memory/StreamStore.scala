...
class StreamStore(bufSize: Int, NumIns: Int)(implicit p: Parameters)
  ...
  val RegStoreQueue = Module(new StoreMIMOQueue(UInt(p(XLEN).W), bus_width, bus_width, bus_width, pipe=true))

  storeQueue.io.deq <> RegStoreQueue.io.enq
  ...
  switch(state){
    ...
    is(sLoading){
      when((storeQueue.io.count + RegStoreQueue.io.count) > (math.pow(2, mp.lenBits).toInt * bus_width).asUInt()){
        len := math.pow(2, mp.lenBits).toInt.asUInt() - 1.U
        state := sSendReq
      }.elsewhen(last){
        len := Mux ((storeQueue.io.count + RegStoreQueue.io.count) % bus_width.U === 0.U, (storeQueue.io.count+RegStoreQueue.io.count)/bus_width.U - 1.U, (storeQueue.io.count+RegStoreQueue.io.count)/bus_width.U)
        state := sLastReq
      }
    }
    is(sSendReq){
...
//  storeQueue.io.deq.ready := io.vme_wr.data.ready
  ...
  io.vme_wr.data.bits := RegStoreQueue.io.deq.bits.asUInt()
  io.vme_wr.data.valid := RegStoreQueue.io.deq.valid
  RegStoreQueue.io.deq.ready := io.vme_wr.data.ready
}
