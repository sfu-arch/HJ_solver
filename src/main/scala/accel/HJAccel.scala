...
class HJAccel(bit_width: Int, mantissa_width: Int, x_size: Int, y_size: Int,
              v_size: Int, theta_size: Int, max_accel: Int, max_w: Int)(implicit p: Parameters) extends Module {
...
//  val core = Module(new DNNCoreTest)
  val core = Module(new HJsolverCore(bit_width= bit_width, mantissa_width= mantissa_width, x_size = x_size, y_size= y_size, v_size= v_size, theta_size= theta_size, max_accel= max_accel, max_w= max_w))
...
/* Receives a counter value as input. Waits for N cycles and then returns N + const as output */
class HJAccelF1(bit_width: Int, mantissa_width: Int, x_size: Int, y_size: Int,
                v_size: Int, theta_size: Int, max_accel: Int, max_w: Int)(implicit p: Parameters) extends Module {
  val io = IO(new Bundle {
