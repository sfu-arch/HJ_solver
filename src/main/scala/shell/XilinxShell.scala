...
/** XilinxShell.
  ...
  * therefore we can pack VTA as an IP for IPI based flows.
  */
class F1Shell(bit_width: Int, mantissa_width: Int, x_size: Int, y_size: Int,
              v_size: Int, theta_size: Int, max_accel: Int, max_w: Int)(implicit p: Parameters) extends RawModule {
  val hp = p(ShellKey).hostParams
  val mp = p(ShellKey).memParams
  ...
  val shell = withClockAndReset(clock = ap_clk, reset = ~ap_rst_n) {
    Module(new HJAccelF1(bit_width= bit_width, mantissa_width= mantissa_width, x_size = x_size, y_size= y_size,
