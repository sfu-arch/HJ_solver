...
$(chisel_build_dir)/$(TOP).v: $(shell find . -type f -name "*.scala" | sed 's/ /\\ /g')#install_vta_package
	sbt 'test:runMain accel.${TOP}Main --target-dir $(chisel_build_dir) --top-name $(TOP) --bit_width $(bit_width) --mantissa_width $(mantissa_width) --x_size $(x_size) --y_size $(y_size) --v_size $(v_size) --theta_size $(theta_size) --max_accel $(max_accel) --max_w $(max_w)'
